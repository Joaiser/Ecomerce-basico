...
export const useCarousel = (productsToShow, initialIndex = 0) => {
    ...
    const nextSlide = () => {
        if (currentIndex < productsToShow.length - 4) { 
            setCurrentIndex(currentIndex + 1); // Cambiado de 2 a 1
        } else {
...
export function SelectionTop(){
    ...
    return(
        <section className="container">
           ...
           <ul>
                <li><a href="#" className={selectedCategory === 'Ordenadores' ? 'selected' : ''} onClick={(event) =>
                    handleCategoryClick('Ordenadores', event)}>Ordenadores</a></li>
                <li><a href="#" className={selectedCategory === 'Componentes' ? 'selected' : ''} onClick={(event) => 
                    handleCategoryClick('Componentes', event)}>Componentes</a></li>
                <li><a href="#" className={selectedCategory === 'Móviles' ? 'selected' : ''} onClick={(event) => 
                    handleCategoryClick('Móviles', event)}>Móviles</a></li>
                <li><a href="#" className={selectedCategory === 'Televisores' ? 'selected' : ''} onClick={(event) => 
                    handleCategoryClick('Televisores', event)}>Televisores</a></li>
                <li><a href="#" className={selectedCategory === 'Hogar tech' ? 'selected' : ''} onClick={(event) => 
                    handleCategoryClick('Hogar tech', event)}>Hogar tech</a></li>
            </ul>
            <div id='carousel-container-seleccion'>
                ...
                selectedCategory === 'Todos').map((product, index) => (
                    <article key={index}>
                        <a href={product.link}>
